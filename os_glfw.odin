...
os_init :: proc() {
	...
	state.os.window = glfw.CreateWindow(1400, 800, "Spreadsheet", nil, nil)
...
key_callback :: proc "c" (window: glfw.WindowHandle, key, scancode, action, mods: i32) {
	...
	if action == glfw.PRESS || action == glfw.REPEAT {
		if (mods & glfw.MOD_CONTROL) != 0 || (mods & glfw.MOD_SUPER) != 0 {
			switch key {
			case glfw.KEY_Z:
				state.pending_undo = (mods & glfw.MOD_SHIFT) == 0
				state.pending_redo = (mods & glfw.MOD_SHIFT) != 0
				return
			case glfw.KEY_Y:
		...
		switch key {
		case glfw.KEY_TAB:
			state.pending_cell_nav = (mods & glfw.MOD_SHIFT) != 0 ? {-1, 0} : {1, 0}
			return
