...
r_draw_dependency_highlights :: proc() {
	if !should_show_dependency_highlights() do return
	ec := state.spreadsheet.editing_cell
	precedents, precedents_ok := get_precedents(ec.col, ec.row)
	dependents, dependents_ok := get_dependents(ec.col, ec.row)
	if precedents_ok {
		for p in precedents do r_draw_rect(cell_screen_rect(p[0], p[1]), PRECEDENT_COLOR)
	}
	if dependents_ok {
		for d in dependents do r_draw_rect(cell_screen_rect(d[0], d[1]), DEPENDENT_COLOR)
	}
}
