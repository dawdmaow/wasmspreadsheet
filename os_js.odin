...
key_down_callback :: proc(e: js.Event) {
	...
	case "Tab":
		state.pending_cell_nav = e.data.key.shift ? {-1, 0} : {1, 0}
		return
	case "Escape":
		state.pending_escape = true
		return
	case "ArrowUp":
		state.pending_cell_nav = {0, -1}
		return
	case "ArrowDown":
		state.pending_cell_nav = {0, 1}
		return
	case "KeyZ":
		if e.data.key.ctrl || e.data.key.meta {
			if e.data.key.shift {
				state.pending_redo = true
			} else {
	...
	if e.data.key.ctrl || e.data.key.meta {
		if e.data.key.code == "KeyC" {
			state.pending_copy = true
			return
		}
		if e.data.key.code == "KeyX" {
			state.pending_cut = true
			return
		}
	}
...
mouse_down_callback :: proc(e: js.Event) {
	...
	case 0:
		mu.input_mouse_down(&state.mu_ctx, state.cursor.x, state.cursor.y, .LEFT)
	case 1:
		mu.input_mouse_down(&state.mu_ctx, state.cursor.x, state.cursor.y, .MIDDLE)
	case 2:
		mu.input_mouse_down(&state.mu_ctx, state.cursor.x, state.cursor.y, .RIGHT)
	}
...
mouse_up_callback :: proc(e: js.Event) {
	...
	switch e.data.mouse.button {
	case 0:
		mu.input_mouse_up(&state.mu_ctx, state.cursor.x, state.cursor.y, .LEFT)
	case 1:
		mu.input_mouse_up(&state.mu_ctx, state.cursor.x, state.cursor.y, .MIDDLE)
	case 2:
		mu.input_mouse_up(&state.mu_ctx, state.cursor.x, state.cursor.y, .RIGHT)
	}
